...
`materials/` contains the sales enablement material library:

- `company/` — Company profile, capabilities, team, methodology, differentiators (manual)
- `case-studies/` — Client case studies with frontmatter tags for matching (manual)
- `solutions/` — Solution templates by category: ai-data, consulting, saas (manual)
- `subsidies/` — Government subsidy info, auto-generated by subsidy-scraper
- `clients/` — CRM client index, auto-generated by crm-projection
- `templates/` — PowerPoint templates (.pptx)
- `presentations/` — Generated presentation output

Case studies use YAML frontmatter with: client, industry, solution_type, scale, duration, year, outcome, tags.
Solutions use YAML frontmatter with: name, category, target_industry, typical_duration, typical_budget, tags.
