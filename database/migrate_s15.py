...
def migrate():
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT client_id, decision_maker, champions FROM crm ORDER BY client_id"
            )
            rows = cur.fetchall()
            ...
            for client_id, dm_jsonb, ch_jsonb in rows:
                ...
                jsonb_dm_name = (
                    dm_raw.get("name", "") if isinstance(dm_raw, dict) else ""
                ) or ""
                jsonb_ch_names = sorted(
                    [
                        c.get("name", "")
                        for c in (ch_raw if isinstance(ch_raw, list) else [])
                        if c.get("name")
                    ]
                )
            ...
            if mismatches:
                ...
                for m in mismatches:
                    ...
                    if not m["dm_match"]:
                        print(
                            f"    DM: JSONB='{m['jsonb_dm']}' vs Norm='{m['norm_dm']}'"
                        )
                    if not m["ch_match"]:
                        print(
                            f"    Champions: JSONB={m['jsonb_ch']} vs Norm={m['norm_ch']}"
                        )
            else:
