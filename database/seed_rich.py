...
def seed():
    ...
    ct3 = create_contact(
        "王經理", org_type="client", org_id=c2["id"], title="IT 經理", role="champion"
    )
    ct4 = create_contact(
        "張總監",
        org_type="client",
    ...
    ct6 = create_contact(
        "吳 PM", org_type="partner", org_id=p3["id"], title="專案經理", role="engineer"
    )
    print(
        f"Contacts: {ct1['name']}, {ct2['name']}, {ct3['name']}, {ct4['name']}, {ct5['name']}, {ct6['name']}"
    )
    ...
    i2 = create_intel(
        ...
        input_type="text",
        source_contact_id=ct3["id"],
    )
    i2 = confirm_intel(
        i2["id"],
        json.dumps(
            {
                "pain_points": ["能源監控", "Excel 管理"],
                "budget": "1M+",
                "timeline": "年底",
            }
        ),
    ...
    i3 = create_intel(
        ...
        input_type="text",
        source_contact_id=ct4["id"],
    )
    i3 = confirm_intel(
        i3["id"],
        json.dumps(
            {
                "pain_points": ["AOI 設備更新"],
                "budget": "500K-800K",
                "timeline": "明年 Q1",
            }
        ),
    ...
    create_tbd(
        "B 石化 IT infra 現況", linked_type="client", linked_id=c2["id"], source="skip"
    )
    create_tbd(
        "C 半導體目前 AOI 供應商",
        linked_type="client",
    ...
    create_tbd(
        "D 製造 demo 環境確認", linked_type="client", linked_id=c4["id"], source="skip"
    )
    print("TBDs: 5 items")
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "UPDATE nx_deal SET last_activity_at = %s WHERE id = %s",
                (idle_date, d1["id"]),
            )
        conn.commit()
    ...
    create_meeting(
        title="A 食品 — 技術方案討論",
        meeting_date=f"{today_str}T14:00:00",
        deal_id=d1["id"],
        participants_json=json.dumps([{"id": ct1["id"], "name": ct1["name"]}]),
    )
    create_meeting(
        title="D 製造 — IoT Demo",
        meeting_date=f"{tomorrow_str}T10:00:00",
        deal_id=d4["id"],
        participants_json=json.dumps([{"id": ct5["id"], "name": ct5["name"]}]),
    )
    create_meeting(
        title="B 石化 — 能源監控需求訪談",
        meeting_date=f"{next_week_str}T15:00:00",
        deal_id=d2["id"],
    ...
    create_file(
        ...
        "proposal",
        "B石化_能源監控_概念書.pdf",
        "/uploads/b_petro_energy.pdf",
        source_url="https://drive.google.com/file/d/def456",
    )
    create_file(
        d4["id"], "attachment", "D製造_場地照片.zip", "/uploads/d_mfg_photos.zip"
    )
    # Mark one as parsed
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE nx_file SET parse_status = 'parsed' WHERE file_name LIKE '%NDA%'"
            )
        conn.commit()
