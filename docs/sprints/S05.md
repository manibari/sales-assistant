...
- [ ] 確認 S03 所有頁面可正常顯示（`pages/work_log.py`、`pages/project.py`）
- [ ] 確認 S04 所有頁面可正常顯示（`pages/annual_plan.py`、`pages/crm.py`、`pages/sales_plan.py`）
- [ ] 確認 `components/sidebar.py` 導航功能正常
...
- [ ] S03 已完成（狀態為 `completed`）
- [ ] S04 已完成（狀態為 `completed`）
- [ ] 已閱讀 `docs/DEVELOPMENT_PLAN.md` Section 4（頁面 6、7 設計）
- [ ] 已閱讀 S03 與 S04 的 Retro 教訓
...
| 2 | 停滯警示標紅顯示 | | |
| 3 | 業績預測表金額正確 | | |
| 4 | 設定頁 6 個 text_input 顯示 | | |
| 5 | 設定儲存後側邊欄更新 | | |
| 6 | 無資料時友善提示 | | |
| 7 | 兩頁面 streamlit run 無錯誤 | | |
