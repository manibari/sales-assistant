...
echo "🚀 Starting backend (FastAPI :8000 + Telegram webhook)..."
cd "$PROJECT_ROOT" && set -a && source .env 2>/dev/null && set +a
uvicorn backend.main:app --reload --port 8000 &>/tmp/nexus-backend.log &
disown
...
$BACKEND_OK  && echo "✅ Backend     http://localhost:8000" || echo "❌ Backend     FAILED (check /tmp/nexus-backend.log)"
